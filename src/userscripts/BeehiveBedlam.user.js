...
const VERSION = [0, 2, 5];
checkUserscript("STBG Beehive Bedlam", VERSION, "BeehiveBedlam");
...
function setCatapultAngle(deg) {
	...
	console.debug("setting angle to", deg);
	bounds = canvas.getBoundingClientRect();
	...
	console.debug({ rad, origin, radius, unit });
	mouseMove({
...
function startGame() {
	console.debug("new state", gameState);
...
uWindow.addEventListener("load", () => {
	setTimeout(() => {
		...
		if (typeof canvas === 'undefined' || !canvas) return;
		console.debug("Collected canvas");
